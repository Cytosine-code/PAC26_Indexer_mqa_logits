...
| :-------------------- | :-------- | :-------- | :--------- | :------ | :------ | :-------------- |
| Case 1 (32, 2, 1024)  | 38.219666 | 26.374455 | 641.099136 | 5860.18 | 7798.54 | **204.05×**     |
| Case 2 (128, 1, 4096) | 42.419292 | 28.682253 | 729.706728 | 4322.48 | 6600.30 | **155.60×**     |

## 4. 当前版本总结

最终版本将原始逐 Head、逐 Token 点积改造成 Page 级矩阵计算：

```text
原始 BF16 Q/K
    -> Q/K 按 K-pair 打包
    -> SME: Q[64,128] × Kᵀ[128,64]
    -> Page Score[64,64]
    -> SVE: ReLU + Head Weight + Head Reduction
    -> Logits[64]
```

整个优化过程说明了三个关键问题：

1. Page-by-Page 只是合适的数据框架，本身不保证性能；若在最内层反复执行标量 BF16 转换，性能反而会低于连续 FP32 Baseline。
2. SVE `BFDOT` 消除了显式 BF16 转换，将单点积提升到 512-bit 向量计算，但每个 Head-Token 仍需要一次横向归约。
3. SME `BFMOPA` 一次生成 `16×16` Score Tile，从根本上消除了逐点横向归约；四个 ZA Tile 又让一个 K 向量同时服务全部 64 Heads。配合 SVE Gather Packing 后，Page 矩阵化的计算效率得到充分发挥。

最终版本在保持精度正确的前提下达到：

```text
Case 1: 7798.54 GFLOPS, 137.41 μs
Case 2: 6600.30 GFLOPS, 1307.98 μs
```

相对原始 Baseline 分别加速 204.05 倍和 155.60 倍。
