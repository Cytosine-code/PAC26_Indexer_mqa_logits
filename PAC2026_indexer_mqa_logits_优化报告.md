...
$$
\frac{10.2}{0.038219666}=266.9\times,
$$
...
|---|---:|---:|---:|---:|---:|
| Case 1 | 38.651T | 26.40T | 26.40T | 10.2T | 38.6% |
| Case 2 | 38.651T | 13.20T | 13.20T | 14.7T | 111%* |

\* Case 2 已突破按 206 GB/s 估算的 13.2 TFLOPS 带宽 roof。V12 把 K 读隐藏到 SME 计算下，DRAM 请求流由"每页 K Pack 的突发"变为"贯穿 SME 的平稳流"，有效带宽提升到约 219 GB/s（128 MiB / 584 μs），证明孤立带宽探针的测量值只是该访问模式的一种实现，不是硬上限。

Case 1 的 26.40 TFLOPS 是长时间稳态下的乐观上限。实际 Case 1 约 105 μs，每线程仅处理约 13--14 个 Page，内存和 SME 很难进入长时间稳态；Q Packing、全局 Barrier、第二个 Q 的调用以及固定启动成本占比更高（V12 后 SME 占 53%--60% 已成为主瓶颈），因此不能由 38.6% 的 Roof 利用率推断存在两倍以上可提取空间。
