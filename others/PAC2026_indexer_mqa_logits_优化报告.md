...
### 3.6 V5：消除 Page Scores 中间缓冲（未采纳）
...
### 3.8 V7：收缩输出初始化并预取下一物理 Page(未采纳)
...
### 3.14 V12：SME 计算期间预取下页 K（路线 A）

#### 3.14.1 动机与 Roofline 依据

V10x 的 Phase Timing 显示 K Pack 是主要瓶颈之一（Case 1 约 28%、Case 2 约 51%）。Roofline 剖析表明：

- **Case 1 的 DRAM 利用率仅约 50%**：K 读（约 40 μs）与 SME 计算（约 50 μs）在每页内串行，SME 计算期间该线程的 DRAM 完全空闲。
- Case 2 虽然已进入带宽受限区，但 K 读同样逐页与 SME 串行，存在可重叠窗口。

此前 V7 / V10x.2 的软件预取都在"当前页 K Pack 之前"发出，与当前页的 DRAM 读竞争，实测为中性或负收益（见 3.10.5 / 3.12.2）。路线 A 的关键区别是**把预取时机放到 SME 计算期间**：BFMOPA 在矩阵单元上执行时，该线程的 load/store 单元与 DRAM 完全空闲，正好用来预取下页 K。

#### 3.14.2 实现方案

在 2×2 BFMOPA 内核的 K 循环尾部插入预取：

```asm
prfm pldl2keep, [x19]   ; 预取下页原始 K 的一个 Cache Line
add  x19, x19, #64
```

每轮 64 次、跨 4 个 Token Block 共 256 次 PRFM，恰好完整覆盖下页 16 KiB。`next_k`（下页原始 K 地址）通过新增参数传入，用 callee-saved `x19` 保存（两个入口的栈帧从 64 字节扩到 80 字节）；无下页时 `cbnz` 回退为预取当前 packed 目标，是命中缓存的安全 no-op。下页地址由 C++ 侧按 Page 顺序 peek 计算。

效果：下页的 K Pack 从"DRAM 读约 3 μs"变为"L2 读约 0.3 μs"，DRAM 读被隐藏到矩阵单元计算期间。

#### 3.14.3 实测结果

生产路径（无 Profiler）多轮稳定：

| 测试用例 | V10x 原版 | V12 预取 | 相对变化 |
|---|---:|---:|---:|
| Case 1 | 9.3 TFLOPS | 10.2 TFLOPS | +9.7% |
| Case 2 | 10.4 TFLOPS | 14.7 TFLOPS | +41.3% |

Phase Timing 对比（Profiler 构建）：

| 阶段 | Case 1 V10x | Case 1 V12 | Case 2 V10x | Case 2 V12 |
|---|---:|---:|---:|---:|
| Q Pack | 13.96% | 15.79% | 1.12% | 1.52% |
| K Pack | 28.08% | 13.96% | 51.22% | 25.80% |
| SME | 43.08% | 53.41% | 38.55% | 59.84% |
| Postprocess | 9.34% | 10.94% | 8.36% | 11.73% |
| Mask Init | 5.54% | 5.91% | 0.75% | 1.12% |
| Overhead | 13.09% | 14.92% | 5.08% | 6.69% |
| 墙钟 | 127.7 μs | 114.8 μs | 916.7 μs | 680.3 μs |

K Pack 占比几乎减半（Case 2 墙钟 0.47 ms → 0.18 ms），SME 成为新的主导阶段（53%--60%）。

#### 3.14.4 突破带宽 Roof

Case 2 生产路径 14.7 TFLOPS 对应墙钟约 584 μs，等效 K 读取带宽

$$
\frac{8192\times16{,}384\text{ B}}{584\text{ μs}}\approx 219\text{ GB/s},
$$

超过了此前带宽探针按随机 Page + K Pack 次序测得的 206 GB/s。说明预取让 DRAM 请求流由"每页 K Pack 的突发"变为"贯穿 SME 的平稳流"，改善了行缓冲命中率，实际有效带宽高于孤立探针的测量值。此前按 206 GB/s 估算的 Case 2 roof（13.2 TFLOPS）不再是硬上限。

#### 3.14.5 结论

路线 A 验证了"把 K 读隐藏到 SME 计算下"的判断，是本项目单点收益最大的改动。SME 现在占 Phase Timing 的 53%--60%，成为新的瓶颈，下一步应聚焦 SME 内核效率（当前约 56% 峰值）或把 Q Pack 等剩余固定成本进一步隐藏到 SME 窗口。

### 3.15 V12 定稿：Q-ready 就绪标志与带宽天花板实测

#### 3.15.1 Q-ready：per-row 就绪标志替代 Q Pack 全局 Barrier

V10 的 Page 级并行在 Q Pack 与 Page 阶段之间依赖 `omp for` 的隐式 Barrier。V12 定稿将其替换为 per-row 就绪标志：

1. `omp for schedule(static) nowait`——每个线程打完自己那份 Q 立即进入 Page 循环；
2. 每行 gather 完成后 `__atomic_store_n(..., __ATOMIC_RELEASE)` 发布该行，放在 mask fill **之前**（mask fill 只碰该行输出尾部 `valid_length..max_model_len`，Page worker 从不写该区间）；
3. Page 循环在 batch 入口 `__ATOMIC_ACQUIRE` 等待该 batch 的 `next_n` 行，用 `waited_batch` 去重（`batch_idx` 单向递增，每 batch 只等一次）；
4. 标志用 `std::vector<uint64_t>` + 64 字节 stride（一行一个 cache line，隔开生产者 store 与消费者 spin），单调 generation 免去每次调用清零。

无死锁依据：static 调度下 chunk 编译期固定，任何线程进 Page 循环前必然已发布自己全部行，被等待的 store 来自不会阻塞在此的线程。

收益来源是 Q Pack 行数不均：Case 1 的 64 行 / 38 线程 = 26×2 + 12×1，那 12 个单行线程提早进入 Page 工作。实测 Case 1 **+0.4 TFLOPS**（10.2→10.6）；Case 2 的 128 行 / 38 线程 ≈ 3.37 接近整除，无收益。该改动并入 V12 定稿，无编译开关。

#### 3.15.2 带宽与计算 Roof 的实测细化

为回答"Case 2 是否到顶"，用两个探针在 38 核、`taskset -c 0-37`、内存绑定 node+16 的条件下精确测量：

**计算 roof（`probe_sme_issue_mix`）**：真实 2×2 BFMOPA 内核的指令组合（15 条/4 BFMOPA）在 L1 驻留下实测 **34.945 TFLOPS**，为纯 BFMOPA 峰值 38.651T 的 90.1%。以后算 SME 效率应以 34.9T 做分母。副产品结论：

- V12 的 256 条 PRFM 在发射侧**完全免费**（加 prfm 前后差 0.07%）；
- 降指令数是**负收益**（展开×2 把每 BFMOPA 非 BFMOPA 指令从 2.75 降到 2.00，性能 -1.4%）——与 V8 双缓冲的 +0.86% 一起，从两个独立角度关死"内核指令层面"这条线；
- 发射带宽假设被推翻（D/A=90.1%，非预期的 60%）。

**带宽矩阵（`probe_page_bw_matrix`，校准位 S-LIN=390≈旧 385、R-KPK=210≈旧 206）**：

| 模式 | GB/s | 隔离的问题 |
|---|---:|---|
| S-LIN 顺序页+线性 | 398.8 | 硬件顺序上限 |
| S-KPK 顺序页+跨行 | 357.5 | 跨行序单独代价 -10% |
| R-LIN 随机页+线性 | 356.4 | 随机映射几乎免费 -11% |
| R-KPK 随机页+跨行 | 209.6 | 组合 -47%（超线性） |
| PACE 随机+跨行+预取1页 | 228.3 | 生产复刻 ≈ 生产 219 |
| PACE-LIN 随机+线性+预取1页 | 337.8 | ≈ R-LIN |
| PACE ×2 / ×4 | 220.2 / 208.8 | 预取深度无用且略有害 |

关键结论：

1. **随机页映射几乎免费**（R-LIN=356 ≈ S-LIN=399）。此前 206 的瓶颈是页内 256B 跨行序，不是 Block Table 随机化。
2. **PRFM 机制没问题**（PACE-LIN ≈ R-LIN，线性访问+预取 ≈ 纯线性）。
3. **预取深度死透**（×2/×4 递减，与跨行 demand 读抢 MSHR）。

**一个必须记录的探针教训**：第一版带宽探针测出失真数（S-LIN=160 而非 385），根因是读循环用单个 XOR 累加器（MLP 低约 2.4×）和数据未 mbind 到 node+16。修复（4 累加器 + 绑定）后校准位恢复。**任何微基准数字在信任前必须先有校准位证明探针本身是好的。**

#### 3.15.3 未采纳尝试：K-pack 线性读 + L1 转置

PACE-LIN=338 曾强烈暗示：把 K-pack 的跨行读改成"线性读 → L1 暂存 → ZA 转置从 L1 读"，能让 DRAM 侧达到 R-LIN 的 338 GB/s。已在 `indexer_mqa_logits.h` 完整实施（4 路展开 SVE 线性拷贝，`-DPAC_K_LINEAR_COPY` 宏开关 + asm `cbz` 回退），逐项审计过寄存器/对齐/字节数，拷贝确实执行、结果逐位一致。

实测：Case 1 10.6→10.5（微降）、Case 2 14.7→14.8（噪声内持平）。**无收益，已回退。**

**为什么平（关键洞察，推翻探针结论向生产的迁移）**：生产里 DRAM 侧访问是 **PRFM 流**（SME 窗口内把下页线性预取到 L2），K-pack 读的是 **L2** 而非直接读 DRAM。把跨行转置从 L2 挪到 L1 不改变 DRAM 投递率。PACE x1=228 与生产 219 的吻合是**巧合**：PACE 是"跨行读 DRAM 慢"，生产是"PRFM 投递慢"，不同机制、相近数字。**探针的"读即 DRAM 访问"条件不适用于生产——生产和探针里读与 DRAM 是解耦的。**

#### 3.15.4 结论：最终性能 10.5 / 14.7 TFLOPS

| Case | 性能 | 耗时 | 卡在哪 |
|---|---:|---:|---|
| Case 1 | 10.5 TFLOPS | 约 101 μs | 串行成本墙：compute+post+Q-pack+mask+OpenMP region，每线程仅 13--14 页，进不了稳态 |
| Case 2 | 14.7 TFLOPS | 约 582 μs | **DRAM 对随机页 PRFM 流的投递墙 ~219 GB/s** |

Case 2 的墙不在计算、不在内核指令、不在预取深度、不在 K-pack 读序——在内存控制器对 PRFM 请求流的投递能力。软件层面能撬动的数据流杠杆已全部封顶。

## 4. 阶段性总结

### 4.1 性能演进

从参考实现到 V12，优化路径经历了算法分块、SVE 向量化、SME 矩阵化、数据重排优化、并行调度重构和访存-计算重叠：
...
| V12 | SME 期间预取下页 K（路线 A） | 10.2 TFLOPS | 14.7 TFLOPS |
| **V12 定稿** | + Q-ready 就绪标志（per-row release/acquire） | **10.5 TFLOPS** | **14.7 TFLOPS** |

相对 Baseline，当前最好成绩的累计加速约为

$$
\frac{10.5}{0.038219666}=274.7\times,
$$

$$
\frac{14.7}{0.042419292}=346.5\times.
$$
...
7. **预取的收益取决于时机而非数量。** 在 K Pack 之前预取会与当前页读竞争（V7/V10x.2 为中性或负收益）；把预取下页的 PRFM 放进 SME 计算期间，走的是该线程完全空闲的 DRAM 窗口，256 条 PRFM 完整覆盖下页后让 K Pack 从 DRAM 读变为 L2 读，Case 2 提速 41%（见 3.14）。
8. **固定规模允许有针对性的寄存器分块。** 四 Token Tile 联合后处理利用了 `block_size=64` 与 SVE FP32 16 Lane 的固定关系，在不改变运算量的前提下减少权重广播并增加 ILP。
9. **访存与计算重叠是带宽未饱和算子的主杠杆。** Case 1 的 DRAM 利用率只有约 50%，把 K 读隐藏到 SME 计算下比优化任何单条指令都有效；Roofline 的可达到墙钟应取"访存与计算的最大值"，而不是两段相加（见 3.14）。
10. **生产里的读与 DRAM 访问是解耦的，探针结论不能直接迁移。** 生产 DRAM 侧是 PRFM 流（预取到 L2），K-pack 读的是 L2；探针的"读即 DRAM 访问"条件不成立。PACE x1=228 与生产 219 的吻合是巧合，不是同一瓶颈（见 3.15.3）。
11. **微基准必须先有校准位。** 第一版带宽探针因单累加器（MLP 低 2.4×）和未 mbind node+16 测出失真数（S-LIN=160 而非 385）；修复后校准位才恢复。任何数字在信任前先证明探针本身是对的（见 3.15.2）。
12. **预取深度不是杠杆。** PRFM 流加深到 ×2/×4 反而略降（与 demand 读抢 MSHR）；V12 的 PRFM 已到该机制的上限，瓶颈转移到内存控制器对 PRFM 流的投递能力（见 3.15.2/3.15.4）。
...
|---|---:|---:|---:|---:|---:|
| Case 1 | 34.945T* | 26.40T | 26.40T | 10.5T | 39.8% |
| Case 2 | 34.945T* | 13.20T | 13.20T | 14.7T | 111%** |

\* 计算 roof 用实测的真实指令组合上限 **34.945 TFLOPS**（`probe_sme_issue_mix` 变体 D，为纯 BFMOPA 峰值 38.651T 的 90.1%），而非纯 BFMOPA 峰值。

** Case 2 已突破按 206 GB/s 估算的 13.2 TFLOPS 带宽 roof。V12 把 K 读隐藏到 SME 计算下，DRAM 请求流由"每页 K Pack 的突发"变为"贯穿 SME 的平稳流"，有效带宽提升到约 219 GB/s（128 MiB / 584 μs）。

**Case 2 的真实墙不在上述 Roof 表里，而在 DRAM 对 PRFM 流的投递能力。** 后续探针（见 3.15.2）证明：随机页 + 页内线性的真实读（R-LIN）可达 356 GB/s，但生产以 PRFM 预取时内存控制器只投递约 219 GB/s；线性读 + L1 转置改造 K-pack（3.15.3）无收益，说明瓶颈是 PRFM 流的 DRAM 效率，软件在当前数据流内无法撬动。因此 219 GB/s ≈ 14.7 TFLOPS 是 Case 2 的实际终点。

Case 1 的 26.40 TFLOPS 是长时间稳态下的乐观上限。实际 Case 1 约 101 μs，每线程仅处理约 13--14 个 Page，内存和 SME 很难进入长时间稳态；Q Packing、Page 阶段启动、第二个 Q 的调用以及固定成本占比高（V12 后 SME 占 53%--60% 已成为主瓶颈），因此不能由 39.8% 的 Roof 利用率推断存在两倍以上可提取空间。串行成本受限下界约 59 μs / 18.2T，实测 101 μs 的缺口是未藏住的内存延迟、smstart/smstop 与稳态不可达。
